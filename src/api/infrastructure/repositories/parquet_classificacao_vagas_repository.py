...
class ParquetClassificacaoVagasRepository(IClassificacaoRepository):
        ...
        if self._df is None:
            if self._parquet_path.exists():
                ...
                self._df = pd.DataFrame(columns=[
                    "posicao", "time", "jogos", "vitorias", "empates", 
                    "derrotas", "gols_pro", "gols_contra", "saldo_gols", "pontos",
                    "zona", "status_curto"
                ])
        return self._df
    
    def _row_to_entity(self, row: pd.Series, temporada: Optional[str] = None) -> ClassificacaoVagas:
    ...
    def get_by_posicao(self, posicao: int, temporada: Optional[str] = None) -> Optional[ClassificacaoVagas]:
    ...
    def get_by_time(self, nome_time: str, temporada: Optional[str] = None) -> Optional[ClassificacaoVagas]:
    ...
    def get_by_zona(self, zona: str, temporada: Optional[str] = None) -> List[ClassificacaoVagas]:
    ...
    def get_libertadores(self, temporada: Optional[str] = None) -> List[ClassificacaoVagas]:
    ...
    def get_sul_americana(self, temporada: Optional[str] = None) -> List[ClassificacaoVagas]:
    ...
    def get_rebaixados(self, temporada: Optional[str] = None) -> List[ClassificacaoVagas]:
