...
class ElencoBronzePipeline(BasePipeline):
    ...
    def __init__(self, max_retries: int = 5, retry_delay: int = 5):
        ...
        logger.warning(" bronze_elenco:bronze - Este pipeline faz apenas extração para visualização")
    ...
    def _fetch_with_retry(self, url: str) -> requests.Response:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"Tentativa {attempt + 1}/{self.max_retries} falhou para {url}: {e}. "
                             f"Esperando {wait_time}s...")
                time.sleep(wait_time)
        
        raise last_exception or requests.RequestException(f"Falha após {self.max_retries} tentativas")
        ...
        for team_name, url in TEAMS_URLS.items():
            try:
                ...
                if len(tabelas) < 2:
                    ...
                    console.print(f"[yellow]⚠️ Tabelas não encontradas para {team_name}[/yellow]")
                    continue
                ...
                console.print(f"[green]✅ {team_name}: {len(goleiros)} goleiros, {len(jogadores_campo)} jogadores de campo[/green]")
    ...
    def _save_team_tables_to_file(self, team_data: dict):
        ...
        for team_name, data in team_data.items():
            ...
            if goleiros:
                with open(output_file, "a", encoding="utf-8") as f:
                    f.write("\nGOLEIROS:\n")
                    f.write(f"{'Nome':<30} {'POS':<5} {'Idade':<6} {'Alt':<7} {'P':<7} {'NAC':<10} {'J':<5} {'SUB':<5} {'D':<5} {'GS':<5} {'A':<5} {'FC':<5} {'FS':<5} {'CA':<5} {'CV':<5}\n")
                    for g in goleiros:
                        f.write(f"{g.get('Nome', ''):<30} {g.get('POS', '-'):<5} {g.get('Idade', '-'):<6} {g.get('Alt', '-'):<7} {g.get('P', '-'):<7} {g.get('NAC', '-')[:10]:<10} {g.get('J', '-'):<5} {g.get('SUB', '-'):<5} {g.get('D', '-'):<5} {g.get('GS', '-'):<5} {g.get('A', '-'):<5} {g.get('FC', '-'):<5} {g.get('FS', '-'):<5} {g.get('CA', '-'):<5} {g.get('CV', '-'):<5}\n")
            
            if jogadores:
                with open(output_file, "a", encoding="utf-8") as f:
                    f.write("\nJOGADORES DE CAMPO:\n")
                    f.write(f"{'Nome':<30} {'POS':<5} {'Idade':<6} {'Alt':<7} {'P':<7} {'NAC':<10} {'J':<5} {'SUB':<5} {'G':<5} {'A':<5} {'TC':<5} {'CG':<5} {'FC':<5} {'FS':<5} {'CA':<5} {'CV':<5}\n")
                    for j in jogadores:
                        f.write(f"{j.get('Nome', ''):<30} {j.get('POS', '-'):<5} {j.get('Idade', '-'):<6} {j.get('Alt', '-'):<7} {j.get('P', '-'):<7} {j.get('NAC', '-')[:10]:<10} {j.get('J', '-'):<5} {j.get('SUB', '-'):<5} {j.get('G', '-'):<5} {j.get('A', '-'):<5} {j.get('TC', '-'):<5} {j.get('CG', '-'):<5} {j.get('FC', '-'):<5} {j.get('FS', '-'):<5} {j.get('CA', '-'):<5} {j.get('CV', '-'):<5}\n")
    ...
    def _show_summary(self, console, df: pd.DataFrame, successful_teams: list):
        ...
        for time_nome in sorted(df["Time"].unique()):
            ...
            table.add_row(time_nome, str(goleiros), str(jogadores), str(goleiros + jogadores))
        
        console.print(table)
        console.print(f"\n[dim]Total de jogadores extraídos: {len(df)}[/dim]")
        console.print(f"[dim]Times extraídos com sucesso: {len(successful_teams)}/{len(TEAMS_URLS)}[/dim]")
