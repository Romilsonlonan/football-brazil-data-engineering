...
def run():
    ...
    df['zona'] = df['posicao'].apply(lambda x: get_zona_brasileirao(x, campeao_copa_brasil=False))
    df['status_curto'] = df['posicao'].apply(get_status_curto)
    
    # Adicionar coluna de aproveitamento
    df['aproveitamento'] = df.apply(
        lambda row: round((row['pontos'] / (row['jogos'] * 3)) * 100, 2) if row['jogos'] > 0 else 0.0,
        axis=1
    )
    ...
    # Criar tabela Rich
    table = Table(title="[bold green]Classificação Brasileirão - Vagas[/bold green]", show_header=True, header_style="bold magenta")
    ...
    console.print("\n")
    vagas_table = Table(title="🎯 RESUMO DAS VAGAS", show_header=True, header_style="bold cyan")
    ...
    # Validar credenciais
    if not postgres_user or not postgres_password or not postgres_host or not postgres_db or settings.postgres_port == 0:
        logger.warning("⚠️ Configurações do PostgreSQL não encontradas. Pulando etapa de carga no banco.")
        logger.info("=" * 60)
