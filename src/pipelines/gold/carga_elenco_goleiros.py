"""Pipeline Gold - Carga de Elenco Goleiros para PostgreSQL/Superset.
=================================================================
...
class GoldCargaElencoGoleiros:
    """Pipeline Gold para carga de dados de goleiros para PostgreSQL/Superset."""

    def __init__(self):
        """Inicializa o pipeline Gold."""
        self.table_name = "gold_elenco_goleiros"
        self.schema_name = "public"

    def run(self) -> dict:
        """Executa o pipeline Gold completo."""
        logger.info("=" * 60)
        logger.info("🔄 PIPELINE GOLD - CARGA ELENCO GOLEIROS")
    ...
    def _salvar_parquet(self, df: pd.DataFrame) -> Path:
        ...
        gold_path = settings.gold_path / "elenco_goleiros.parquet"
        ...
        if settings.minio_enabled:
            ...
            minio_path = save_to_minio(df, "gold", "elenco_goleiros.parquet")
            if minio_path:
...
def run() -> dict:
    ...
    pipeline = GoldCargaElencoGoleiros()
    return pipeline.run()
