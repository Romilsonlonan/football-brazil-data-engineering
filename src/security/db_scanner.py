...
class DatabaseScanner:
    ...
    def scan_connection(
        ...
        if not PIICATCHER_AVAILABLE:
            raise ImportError(
                "piicatcher não está instalado. " "Execute: pip install piicatcher"
            )
    ...
    def scan_table(
        ...
        return TableScanResult(
            table_name=table_name, has_pii=False, pii_types=[], columns_with_pii=[]
        )
    ...
    def _mask_connection(self, conn_str: str) -> str:
        ...
        # Substitui senha por ***
        masked = re.sub(r"(://[^:]+:)[^@]+(@)", r"\1***\2", conn_str)
        return masked
...
def scan_database_security(
    connection_string: str, db_name: str = None
) -> DatabaseScanResult:
    """
