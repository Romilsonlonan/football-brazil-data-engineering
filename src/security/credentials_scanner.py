...
class CredentialsScanner:
    ...
    def scan_directory(
        self, directory: str, recursive: bool = True, extensions: Optional[set] = None
        ...
        if result.has_credentials:
            logger.critical(
                f"🚨 ALERTA: {result.credentials_found} credenciais detectadas!"
            )
            self._log_findings(result)
        else:
            logger.info(
                f"✅ Scan concluído: Nenhuma credencial detectada em {result.files_scanned} arquivos"
            )
        logger.info("=" * 60)
        ...
        for cred_type, pattern in self._compiled_patterns.items():
            ...
            for match in matches:
                ...
                finding = CredentialFinding(
                    ...
                    severity=self.severity_override.get(
                        cred_type, self._get_default_severity(cred_type)
                    ),
                )
    ...
    def _log_findings(self, result: CredentialsScanResult):
        ...
        logger.critical("=" * 60)
        logger.critical(
            f"🚨 ALERTA DE SEGURANÇA: {result.credentials_found} credenciais detectadas!"
        )
        logger.critical("=" * 60)
        ...
        for finding in result.findings:
            by_type[finding.credential_type] = (
                by_type.get(finding.credential_type, 0) + 1
            )
