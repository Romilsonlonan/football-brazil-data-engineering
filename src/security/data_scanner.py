...
class DataSecurityScanner:
    ...
    def _setup_custom_recognizers(self):
        ...
        try:
            ...
            cpf_recognizer = PatternRecognizer(
                ...
                patterns=[r"\d{3}\.?\d{3}\.?\d{3}-?\d{2}", r"\d{11}"],
                name="Brazilian CPF",
            ...
            cnpj_recognizer = PatternRecognizer(
                ...
                patterns=[r"\d{2}\.?\d{3}\.?\d{3}\/?\d{4}-?\d{2}", r"\d{14}"],
                name="Brazilian CNPJ",
                supported_language="pt",
            )
            ...
            logger.info(
                "✅ Reconhecedores customizados registrados (CPF, CNPJ, Telefone, PIS, CNH)"
            )
    ...
    def scan_dataframe(
        ...
        for column in df.columns:
            column_findings = self._scan_column(
                df[column], column, table_name, entities
            )
    ...
    def _scan_column(
        self, series: pd.Series, column_name: str, table_name: str, entities: List[str]
    ...
    def _log_findings(self, result: SecurityScanResult):
        ...
        if result.has_risks:
            logger.critical("=" * 60)
            logger.critical(
                f"🚨 ALERTA DE SEGURANÇA: {result.risks_found} dados sensíveis detectados!"
            )
            logger.critical("=" * 60)
            ...
            logger.critical("=" * 60)
            logger.critical(
                "⚠️  AÇÃO NECESSÁRIA: Revise e anonimize os dados antes de prosseguir!"
            )
            logger.critical("=" * 60)
        else:
            logger.info(
                f"✅ Scan concluído: Nenhum dado sensível detectado em {result.table_name}"
            )
            logger.info(
                f"   Total de {result.total_rows} linhas verificadas em {result.columns_scanned} colunas"
            )
    ...
    def anonymize_text(self, text: str, entities: Optional[List[str]] = None) -> str:
        ...
        try:
            analyzer_results = self.analyzer.analyze(
                text=text, language=self.language, entities=entities
            )
...
def with_security_scan(table_name: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            logger.info(
                f"🔒 Executando pipeline com verificação de segurança: {table_name}"
            )
