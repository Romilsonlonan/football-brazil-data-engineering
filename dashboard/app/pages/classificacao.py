...
from dashboard.app.components.charts import (
    bar_chart,
    pie_chart,
    g4_donut,
    top10_bar,
    create_zone_probability_chart,
)
...
_ZONE_BUTTONS = [
    ...
    {
        "label": "G4",
        "title": "Libertadores - fase de grupos",
        "color": "#3fb950",
        "id": "zone-g4",
    },
    {
        "label": "G6",
        "title": "Libertadores - incluindo pré",
        "color": "#2ea043",
        "id": "zone-g6",
    },
...
def _normalize_time_name(name: str) -> str:
    """Normaliza nome do time para busca no DataFrame."""
    replacements = {
...
def _render_time(df: DataFrame, time: str) -> html.Div:
    normalized = _normalize_time_name(time)
    df_time = df[df["time"].str.contains(normalized, case=False, na=False)]
    if df_time.empty:
        return html.Div(
            className="page-header",
            children=[
                html.H1(f"📊 {time}"),
                html.P("Time não encontrado nos dados"),
            ],
        )
    row = df_time.iloc[0]
    ...
    # Calcular zona
    if posicao <= 4:
        zona = "🟢 G4 (Libertadores)"
    elif posicao == 5 or posicao == 6:
        zona = "🔵 G6 (Pré-Libertadores)"
    elif posicao <= 14:
        zona = "🌎 G14 (Sul-Americana)"
    else:
        zona = "🔴 Z4 (Rebaixamento)"
    ...
    return html.Div(
        children=[
            ...
            html.Div(
                ...
                children=[
                    metric_card("📍", f"#{posicao}", zona, highlight=posicao <= 6),
                    metric_card("⭐", str(int(row["pontos"])), "Pontos"),
                    metric_card("🏆", str(int(row["vitorias"])), "Vitórias"),
                    metric_card("⚽", str(int(row["gols_pro"])), "Gols Marcados"),
                ],
            ),
            # Gráfico de probabilidade de zonas
            html.Div(
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                children=[
                                    html.Div(
                                        "🎯 Probabilidade de Zonas",
                                        className="chart-title",
                                    ),
                                    html.Div(
                                        f"{time} - Probabilidades",
                                        className="chart-subtitle",
                                    ),
                                ]
                            ),
                            dcc.Graph(
                                figure=create_zone_probability_chart(posicao, df),
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                children=[
                                    html.Div(
                                        "📊 Posição na Tabela", className="chart-title"
                                    ),
                                    html.Div(
                                        f"{time} - Desempenho",
                                        className="chart-subtitle",
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    metric_card(
                        "⚽", f"{int(df['gols_pro'].sum()):,}", "Gols Marcados"
                    ),
                ],
            ),
            html.Div(
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                "🍩 G4 - Top 4 Brasileirão", className="chart-title"
                            ),
                            dcc.Graph(
                                figure=g4_donut(df), config={"displayModeBar": False}
                            ),
                        ],
                    ),
                    html.Div(
            ...
            html.Div(
                className="chart-card",
                style={"gridColumn": "span 2", "marginBottom": "30px"},
                children=[
