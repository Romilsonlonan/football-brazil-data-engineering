    ...
    if time == "Todos":
        ...
        for col, name, color in [
            ...
            fig.add_trace(go.Bar(
                y=top5["time"], x=top5[col], name=name,
                marker_color=color, orientation="h",
                text=top5[col], textposition="inside",
            ))
        fig.update_layout(
...
def pie_chart(df: DataFrame, time: str = "Todos") -> go.Figure:
    if time == "Todos":
        values = [int(df["vitorias"].sum()), int(df["empates"].sum()), int(df["derrotas"].sum())]
    else:
        row = df[df["time"].str.contains(time, case=False)].iloc[0]
...
def top10_bar(df: DataFrame, filtro: str = "all") -> tuple[go.Figure, str]:
    mapa = {
        "libertadores": (df.head(6), "🏆 Libertadores (1-6)"),
    ...
    fig = go.Figure(data=[go.Bar(
        x=dados["time"], y=dados["pontos"],
        marker=dict(color=dados["pontos"], colorscale="Viridis", showscale=False),
        text=dados["pontos"], textposition="outside",
        textfont=dict(color="white", size=11),
    )])
    fig.update_layout(
        xaxis=dict(title="", tickfont=dict(color="white", size=10), gridcolor="rgba(255,255,255,0.1)"),
        yaxis=dict(title=dict(text="Pontos", font=dict(color="white")), tickfont=dict(color="white"),
